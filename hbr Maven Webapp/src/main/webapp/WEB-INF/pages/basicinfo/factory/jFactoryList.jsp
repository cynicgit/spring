...
<table id="ec_table" class="tableRegion" width="98%" >
	...
	<c:forEach items="${factoryList}" var="o" varStatus="status">
		<tr class="odd" onmouseover="this.className='highlight'" onmouseout="this.className='odd'" >
			...
			<td>${o.inspector}</td>
			<td>
				<c:if test="${o.state==0 }"><a href="start.action?id=${o.id}"><span style="color: orange;">停用</span></a></c:if>
				<c:if test="${o.state==1 }"><a href="stop.action?id=${o.id}"><span style="color: orange;">启用</span></a></c:if>
			</td>
		</tr>
	</c:forEach>
